...
**omni 判断模式**（`server.omni.judge_mode`——⚙️ 面板可切，**两方案并存独立**）：

| 模式 | 输入 | 时延 | 特点 |
|------|------|------|------|
| text（默认——老方案） | ASR 转写文本（ASR 后判断） | 串行（ASR + 判断 ~0.6s） | 文本干净（无回声/噪声干扰）；依赖 ASR 正确性 |
| audio（新方案——实验） | **当前轮音频**（multimodal-generation）+ 文本上下文（播放状态/播放文本/历史） | **与 ASR 并行**（实测 449ms vs 文本 636ms） | ASR 错误免疫、声学信息不丢（语气/停顿）；音频推理本身慢——并行隐藏；失败回退文本判断 |

- 音频直判 API：`/api/v1/services/aigc/multimodal-generation/generation`（compatible-mode 不支持音频输入——400 实测）
- 上下文设计复用：播放状态 {playing}/播放文本/对话历史（3 轮）——当前轮替换为音频
